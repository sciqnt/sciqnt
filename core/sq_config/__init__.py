...
SCHEMA: list[Setting] = [
    ...
    Setting(
        key="demo_mode", default="auto", type="enum",
        allowed=["auto", "on", "off"], group="general",
        help="The demo portfolio (synthetic, deterministic — sciqnt's "
             "public figures). auto = shown only while no real account is "
             "connected; on = always; off = never.",
        consumed_by="sq_platform.aggregated (_discover_brokers void-fill)",
    ),
]
